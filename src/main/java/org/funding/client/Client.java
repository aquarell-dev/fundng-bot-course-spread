package org.funding.client;

import org.funding.orderbook.Orderbook;
...
public abstract class Client extends WebSocketClient {
    ...
    public Client(Exchange exchange, TickersManager tickersManager) {
    ...
    public boolean areOrderBooksFilled() {
        ...
        return orderBooks.values().stream().allMatch(this::isOrderBookFilled);
    }

    public List<String> getUnfilledOrderBooks() {
        return Collections.singletonList(String.join(", ", orderBooks.values().stream().filter(ob -> !isOrderBookFilled(ob)).map(orderBook -> orderBook.coin).toList()));
    }
